...
*******************************************************************************/
MAT_SHIN_STRENGTH::MAT_SHIN_STRENGTH ()
{
...
* Name:			Read3DS ()
*******************************************************************************/
int MAT_SHIN_STRENGTH::Read3DS (int iNrOfBytes, byte* bp3DSData)
{
	...
	// Print status
	VerbosePrintf ("Begin reading MAT_SHIN_STRENGTH chunk");
	...
	// Get chunks from MAT_SHIN_STRENGTH chunk
	while (iCurrentByte < iNrOfBytes)
	{
		...
		switch (iChunkID)
		{
			case INT_PERCENTAGE_CHUNK:
				...
				if (MAT_SHIN_STRENGTH::pIntPercentage.Read3DS (lChunkLength - 
			...
			default:
				// Print status
				VerbosePrintf ("Unknown chunk found in MAT_SHIN_STRENGTH: %x", iChunkID);
				break;
		}
	...
	// Print status
	VerbosePrintf ("End reading MAT_SHIN_STRENGTH chunk");
...
* Name:			GetShininess ()
*******************************************************************************/
int MAT_SHIN_STRENGTH::GetShininess (int* iPercentage)
{
	int 	iPer;		// Percentage
